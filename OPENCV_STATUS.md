...
2. **Environment Variable**
   - Set in Supabase: `OPENCV_SERVICE_URL=http://localhost:8000`
   - For production: Use deployed OpenCV service URL

## Known Limitations

1. **Image Quality**: Results depend on image quality and lighting
2. **Container Detection**: Works best with white/light-colored containers
3. **Estimation Accuracy**: pH and turbidity are estimates, not lab-grade measurements
4. **Development Server**: Currently using Flask development server (not for production)

## Production Deployment

For production, deploy the OpenCV service to:
